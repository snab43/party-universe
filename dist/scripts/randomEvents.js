...
(function (randomEvents) {
    function friendZonePost(militaryTime) {
        if (Math.random() <= 1 / 200) {
            ...
            Social.updateFriendZoneFeed(chance.name(), chance.city() + ", " + chance.country(), post, militaryTime);
        }
        if (Math.random() <= 1 / 500) {
            Social.updateFriendZoneFeed("Advertisement", "Sponsored", DIALOGUE_FRIENDSPACE_AD[Math.floor(Math.random() * DIALOGUE_FRIENDSPACE_AD.length)], militaryTime);
        }
    }
    randomEvents.friendZonePost = friendZonePost;
    function friendZonePostLiked() {
        var _a;
    ...
    randomEvents.friendZonePostLiked = friendZonePostLiked;
